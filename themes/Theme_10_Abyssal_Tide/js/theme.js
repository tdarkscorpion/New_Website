// Abyssal Tide & Ocean Interactive Particle Engine (abyssal-canvas)
(function() {
  ...
  function initThemeEngine() {
    ...
    const canvas = document.getElementById('abyssal-canvas');
    if (!canvas) return;
    initialized = true;
    ...
    const colors = ["#14b8a6", "#06b6d4", "#5eead4", "#2dd4bf"];
    ...
    function animate() {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        p.y -= p.speedY; p.x += p.speedX; p.size = Math.max(0.8, p.size + (Math.random()-0.5)*0.1);
